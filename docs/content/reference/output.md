...
describes what users get. Pick a format with `-o`, or let pydocs choose:
a table when writing to a terminal, JSONL when piped.

## Formats

```bash
pydocs <command> -o table   # aligned columns for reading
pydocs <command> -o jsonl   # one JSON object per line, for piping
pydocs <command> -o json    # a single JSON array
pydocs <command> -o csv     # spreadsheet friendly
pydocs <command> -o tsv     # tab-separated
pydocs <command> -o url     # just the URL column
pydocs <command> -o raw     # the underlying bytes, unformatted
```
...
```bash
pydocs <command> --fields id,title,url
```
...
```bash
pydocs <command> --template '{{.URL}} {{.Title}}'
```
...
```bash
pydocs <command>            # a table, because this is a terminal
pydocs <command> | wc -l    # JSONL, because this is a pipe
```
